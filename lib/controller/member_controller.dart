...
class MemberController extends GetxController {
  ...
  validationEdit({BuildContext? context, void callback(result, exception)?}) async{
    if (edtbank.text == '') {
      DialogConstant.alertError('Bank tidak kosong!');
    } else if (edtnorek.text == '') {
      DialogConstant.alertError('Nomor Rekening tidak boleh kosong!');
    } else {
      editmember(
  ...
  editmember({BuildContext? context, void callback(result, exception)?}) async {
    var post = new Map<String, dynamic>();
    var header = new Map<String, String>();
    SharedPreferences preferences = await SharedPreferences.getInstance();
    ...
    API.basePostGolang(ConstUrl.upddatabank, post, header, true, (result, error) {
        Get.back();
        if (error != null) {
          callback!(null, error);
        }
